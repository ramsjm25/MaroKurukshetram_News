...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (type) {
      // Check if this is a request to e-newspapers endpoint with type parameter
      const url = new URL(req.url, `http://${req.headers.host}`);
      const pathname = url.pathname.replace('/api', '');
      
      if (pathname.startsWith('/e-newspapers')) {
        // This is an e-newspapers request, route it directly regardless of type parameter
        targetUrl = `${apiBaseUrl}/e-newspapers`;
        logPrefix = '[E-Newspapers Direct API]';
        console.log(`${logPrefix} E-newspapers request with params:`, query);
      } else {
        // Handle other type-based routing
        switch (type) {
          case 'languages':
          ...
          default:
            return res.status(400).json({ 
              status: 0, 
              message: 'Invalid type. Must be one of: languages, categories, states, districts, local-mandis, roles, health-check, e-newspapers',
              result: null 
            });
        }
      }
      ...
      console.log(`[Direct Path Routing] URL: ${req.url}, Pathname: ${pathname}`);
      ...
      if (pathname.startsWith('/news/') || pathname.startsWith('/auth/') || pathname.startsWith('/local-mandi-categories') || pathname.startsWith('/e-newspapers') || pathname.startsWith('/local-mandis')) {
        ...
        console.log(`[Direct Path Routing] Matched path: ${pathname}, Target URL: ${targetUrl}`);
        ...
        if (pathname.startsWith('/e-newspapers')) {
          logPrefix = '[E-Newspapers Direct API]';
          console.log(`[E-Newspapers Direct API] Processing request with params:`, query);
        }
      } else {
        console.log(`[Direct Path Routing] No match for pathname: ${pathname}`);
        return res.status(400).json({ 
          status: 0, 
    ...
    // For e-newspapers, try to add a default authorization if none is present
    if (logPrefix.includes('E-Newspapers') && !req.headers.authorization) {
      // Try to use a default token or skip authentication for public newspapers
      console.log(`${logPrefix} No authorization header found, attempting public access`);
    }
    ...
    try {
      ...
      // Special handling for e-newspapers API
      if (logPrefix === '[E-Newspapers API]' && data.status === 1 && data.result) {
        console.log(`${logPrefix} Processing e-newspapers results: ${data.result.items?.length || 0} items`);
        
        if (!data.result.items || data.result.items.length === 0) {
          console.log(`${logPrefix} No e-newspapers found for the given parameters`);
        } else {
          console.log(`${logPrefix} Found ${data.result.items.length} e-newspapers`);
          // Log each newspaper for debugging
          data.result.items.forEach((newspaper, index) => {
            console.log(`${logPrefix} Newspaper ${index + 1}:`, {
              date: newspaper.date,
